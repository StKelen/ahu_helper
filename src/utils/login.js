// Wafer默认的登录模块。仅对登录系统进行修改，以支持登录参数
/**
 * README!!!
 * 为了兼容微信修改的登录逻辑
 * 这里对登录的 SDK 进行重构
 ...
 */
function getWxLoginResult (callback) {
    wx.login({
        success (loginResult) {
            wx.getUserInfo({
                success (userResult) {
                ...
                fail (userError) {
                    callback(new Error('获取微信用户信息失败，请检查网络状态'), null)
                }
            })
        },
        fail (loginError) {
            callback(new Error('微信登录失败，请检查网络状态'), null)
        }
    })
}
...
export default { login, setLoginUrl, loginWithCode }
